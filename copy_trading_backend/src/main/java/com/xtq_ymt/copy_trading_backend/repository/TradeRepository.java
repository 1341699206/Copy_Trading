...
public interface TradeRepository extends JpaRepository<Trade, Long> {
    ...
    // 已有方法：获取账户的未平仓交易
    List<Trade> findByAccountIdAndIsClosedFalse(Long accountId);

    // 已有方法：获取与策略相关的所有交易
    List<Trade> findByStrategyId(Long strategyId);

    // 新增方法：获取交易员的总收益
    @Query("SELECT SUM(t.profit) FROM Trade t WHERE t.accountId = :accountId AND t.isClosed = true")
    Double getTotalProfitByTraderId(@Param("accountId") Long accountId);

    // 新增方法：获取交易员的总交易数量
    @Query("SELECT COUNT(t) FROM Trade t WHERE t.accountId = :accountId")
    Integer getTotalTradesByTraderId(@Param("accountId") Long accountId);

    // 新增方法：获取交易员的盈利交易数量
    @Query("SELECT COUNT(t) FROM Trade t WHERE t.accountId = :accountId AND t.profit > 0")
    Integer getWinningTradesByTraderId(@Param("accountId") Long accountId);

    // 新增方法：获取交易员的最大回撤
    @Query("SELECT MAX(t.profit) - MIN(t.profit) FROM Trade t WHERE t.accountId = :accountId AND t.isClosed = true")
    Double getMaxDrawdownByTraderId(@Param("accountId") Long accountId);

    // 新增方法：获取跟随者的总收益
    @Query("SELECT SUM(t.profit) FROM Trade t WHERE t.accountId = :followerId AND t.isClosed = true")
    Double getTotalProfitByFollowerId(@Param("followerId") Long followerId);

    // 新增方法：获取跟随者的总交易数量
    @Query("SELECT COUNT(t) FROM Trade t WHERE t.accountId = :followerId")
    Integer getTotalTradesByFollowerId(@Param("followerId") Long followerId);

    // 新增方法：获取跟随者的最大回撤
    @Query("SELECT MAX(t.profit) - MIN(t.profit) FROM Trade t WHERE t.accountId = :followerId AND t.isClosed = true")
    Double getMaxDrawdownByFollowerId(@Param("followerId") Long followerId);

    // 新增方法：获取跟随者总共跟随的交易员数量
    @Query("SELECT COUNT(DISTINCT t.strategyId) FROM Trade t WHERE t.accountId = :followerId")
    Integer getTotalFollowedTraders(@Param("followerId") Long followerId);

    // 新增方法：获取利润最高的前 N 名交易员
    @Query("SELECT t.accountId, SUM(t.profit) FROM Trade t GROUP BY t.accountId ORDER BY SUM(t.profit) DESC")
